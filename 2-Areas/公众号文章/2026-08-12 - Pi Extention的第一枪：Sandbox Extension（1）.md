---
title: "Pi Extention的第一枪：Sandbox Extension（1）"
author: "AI萝卜"
publish_date: "2026-08-12 06:00:00"
saved_date: "2026-08-16"
source: "wechat"
url: "https://mp.weixin.qq.com/s/ki6a0nTkSV5xEN5FIdSCQg"
tags:
  - AI-Agent-研究
  - Sandbox
  - Pi
  ...
  - 德勤-可借鉴
---
# Pi Extention的第一枪：Sandbox Extension（1）
**Pi 加冕之路 · 已发布**

- [亲手写个 mini-pi，再对照 pi 的 743 行源码](http://mp.weixin.qq.com/s?__biz=MzkwNDY0ODk1Ng==&mid=2247483885&idx=1&sn=bdd9f1faa0c054cb3dbb1e304be8db22&scene=21#wechat_redirect)
- [读懂Pi，你就是AI应用之王](http://mp.weixin.qq.com/s?__biz=MzkwNDY0ODk1Ng==&mid=2247483880&idx=1&sn=6eadf0ce3e021904b69e4dd5536a294a&scene=21#wechat_redirect)
- [Pi的加冕之路：循环之外的调度机制](http://mp.weixin.qq.com/s?__biz=MzkwNDY0ODk1Ng==&mid=2247483870&idx=1&sn=057760b8f519adc810a96c5f626a8015&scene=21#wechat_redirect)
- [Pi加冕之路：EventStream实时事件通信机制](http://mp.weixin.qq.com/s?__biz=MzkwNDY0ODk1Ng==&mid=2247483861&idx=1&sn=b43f7e79a8cba7ef6bcd66c143c43ab3&scene=21#wechat_redirect)
- [Pi的扩展系统：什么都不内置怎么长出一切](http://mp.weixin.qq.com/s?__biz=MzkwNDY0ODk1Ng==&mid=2247483861&idx=1&sn=b43f7e79a8cba7ef6bcd66c143c43ab3&scene=21#wechat_redirect)

> **临江仙 · 沙箱**自律从来难自缚， 纸条贴遍门窗。 手机在兜誓不刷， 三更破了戒， 还怨意志伤。须知锁匙交他手， 内核铁壁高墙。 任凭代码百般狂， 柜员摇头处， 一句不商量。

---

# Pi 的沙箱：一个不存在的内置功能

上一篇我们聊了 [Pi 的扩展系统：什么都不内置怎么长出一切](http://mp.weixin.qq.com/s?__biz=MzkwNDY0ODk1Ng==&mid=2247483891&idx=1&sn=418d215b22b3c21968b2fa7ea024e3e2&token=1890475646&lang=zh_CN&scene=21#wechat_redirect)——Pi 自己不干活，全靠扩展干。那你可能想问：Pi 有沙箱吗？它怎么保护你的系统不被 AI 搞崩？
...
- 你的代码准备好参数（"我想打开 `/tmp/hello.txt`，只读模式"）
- 执行一条特殊 CPU 指令，**把控制权交给内核**
- 内核——另一个程序，跑在 CPU 的特权模式里——决定行不行
- 内核干活（或者拒绝），把结果还给你
这整个 1→2→3→4 的流程叫一次 **syscall**（系统调用）。你 Mac 的 ARM64 芯片上，那条特殊指令是 `svc #0x80`（supervisor call，管理员调用，下面有更形象的展示，往下看👇）。像去银行柜台办业务：你填个单子递进去，柜员决定办不办、怎么办，你全程只能等结果。钱（文件、网络、硬件）全在柜台里面，你摸不到。
...
整个 syscall 就是"填好单子（寄存器），递给柜员（`svc #0x80`），等结果"。没有 HTTP 包，没有 JSON。

Python 的 `open("/tmp/hello.txt")` 最后也落到这同一组"往格子里填数字 + `svc #0x80`"上——只是中间裹了很多层让它好用的库代码。
...
操作发生了什么`pip install pkg`可能执行 `setup.py`，编译 C 代码，跑任意命令`npm install``postinstall` 脚本什么都能做启动 language server一个拥有完整权限的长驻进程触发 git hook`pre-commit`、`post-checkout` 都是任意脚本每一个都是智能体*应该*能做的合法操作。但每一个产生的新进程都不知道"沙箱"是什么玩意儿。限制只活在智能体自己的内存里——不传播、不继承、不强制。
...
全部失败。`open()`、`os.open()`、生成子进程 `touch`——全都落到同一个 `svc #0x80`，内核在*那个层级*检查策略。进程没有绕过的可能，因为检查点在它物理上无法访问的地方。
...
![](https://mmbiz.qpic.cn/mmbiz_png/zh1fELX24eH7I8geLCx38iaV8PZtvE0PElM4qNpMXFGMQh6b2KJdeWL9TC93D3kibHIEQCichrxQdjxpF3jMJ2y4iadibgbaBgZia0fEQxicdL74Sc/640?wx_fmt=png&from=appmsg)
...
- **替换内置的 bash tool** — 扩展注册了一个同名工具，在执行前把命令交给 `SandboxManager.wrapWithSandbox()` 处理
- **OS 级包装** — 在 macOS 上用 `sandbox-exec`（Seatbelt），在 Linux 上用 `bubblewrap`（用户命名空间）
- **策略可配置** — 通过 JSON 配置文件定义允许的网络域名、文件读写路径
有一点要特别说清楚：**Pi 不会拦命令本身——命令一定会跑。** 不会弹窗问你"允许执行 `rm -rf /` 吗？"，不会在执行前审批。它的做法是让命令跑起来，但跑的环境被锁死了：

命令会发生什么`rm -rf /`直接执行，但只能删 `allowWrite` 配置的路径（当前目录和 `/tmp`），其他地方内核返回 `Operation not permitted``curl evil.com`直接执行，但代理拒绝连接，命令报网络错误`cat ~/.ssh/id_rsa`直接执行，但内核拒绝读取，返回 `Operation not permitted`对比 Claude Code 的做法：Claude Code 会在执行前问你"允许吗？"——相当于门口站保安。Pi + sandbox extension 不问，直接让你进，但房间里的抽屉锁上了、窗户封了。两种思路，同样安全，交互方式完全不同。

整体流程：

![](https://mmbiz.qpic.cn/sz_mmbiz_png/zh1fELX24eHvq6Xz1vFTiaYicDyHwhPib3ybwPibhS7g47b3uvL48YSJD03Z4ySibBCzluKVaIs9x2WDjtsm5tUsdWwJuV8UheiaSDPeibayKHKKYk/640?wx_fmt=png&from=appmsg)
...
- **承认现实** — 一个需要跑 `npm install`、`pip install`、`git commit`、启动 language server 的工具，不可能在进程内做出有意义的限制。能做的是：把需要权限的操作*委托*给一个被内核管着的子进程。这正是 sandbox extension 做的事。

![](https://mmbiz.qpic.cn/sz_mmbiz_png/zh1fELX24eFOUsljzZqiaj7cW7fVVCjibMcYI9rDueNpfuxXtDibhW93sibtfncY8kczs01GndV0KAaYH0KcZHicK89IVyqJzpaq9ApmE8rhtIzc/640?wx_fmt=png&from=appmsg)

Pi sandbox extension 架构

![](https://mmbiz.qpic.cn/mmbiz_png/zh1fELX24eElPKK7uk21rpfHBbibMFhtIfgmhvAclGib3Lomzn1ibqPPrPk1BfGMlmOIR9mC717lRrHgkeUmneKo0jAXpYwcdMib3110vGWcpXs/640?wx_fmt=png&from=appmsg)
